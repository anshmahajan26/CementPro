...
const SavedCarbonsManager = ({
  ...
  const fetchSaved = async () => {
    try {
      setLoading(true);
      const res = await api.get("/carbon/saved");
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true);
      await api.delete(`/carbon/saved/${id}`);
      fetchSaved();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-card w-full max-w-2xl rounded-xl shadow-xl border border-border overflow-hidden flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between p-4 border-b border-border bg-muted/40">
          <h2 className="text-xl font-bold">Saved Carbon Profiles</h2>
        ...
        <div className="p-4 overflow-y-auto flex-1">
          <h3 className="text-sm font-semibold mb-3">Your Carbon History</h3>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        onLoadCarbon(unifiedFeatures, item.forecast_days, item.blend_factor, item.realtime_mode, item.results, item.name);
                        onClose();
                      }}
                    >
                      Load Model 
...
export default SavedCarbonsManager;
